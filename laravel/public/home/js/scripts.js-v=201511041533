...
function ajaxIsLogin(resp, backUrl) {
  ...
  if(resp.status === '-100') {
    if (backUrl) {
      window.location.href = OPPO.conf.ACCOUNT_URL + '/auth/login?callback=' + backUrl;
    } else {
      window.location.href = OPPO.conf.ACCOUNT_URL + '/auth/login?callback=' + window.location.href;
    }
  } else {
    return true;
  }
